...
%       have the function signature [cost] = lstage(x,u)
%   (TODO) * lterm - a casadi Function which describes the terminal cost
%       (i.e., the cost incurred at the end of the prediction horizon);
%       this should have the function signature [cost] = lterm(x,u)
%
% for an ECONOMIC MPC, the following must be additionally defined (or kept
% as given):
%
% for the SIMULATION, the following must be defined:
%   * nx, nu, ny, nyc, Np - see above
%   * if the system was augmented in any way, and additional states must be
%       computed (e.g., CEM (plasma dose), additional functions should be
%       defined); i.e., CEMadd was defined here
%   * nv - output noise
%   * nw - process noise
